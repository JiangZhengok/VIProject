	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0)
	    {
	    	...
	    	{
			    sensor_msgs::PointCloud msg_match_points;  //发送给estimator的匹配信息
			    ...
			    for (int i = 0; i < (int)matched_2d_old_norm.size(); i++)
			    {
		            ...
		            p.z = matched_id[i];
		            msg_match_points.points.push_back(p); //points - 匹配道的角点的归一化坐标和地图点的id
			    }
			    ...
			    msg_match_points.channels.push_back(t_q_index); //channels - 回环帧的pose
