...
from xsens_driver.msg import mti #(@jiang)
...
class XSensDriver(object):
    ...
    def reset_vars(self):
        ...
        self.euler_msg = mti()
        self.euler_msg.yaw = 0.0
        self.euler_msg.pitch = 0.0
        self.euler_msg.roll = 0.0
        self.pub_euler = False
    ...
    def spin_once(self):
        ...
        def fill_from_Orientation_Data(o):
            ...
            try:
                x, y, z, w = quaternion_from_euler(radians(o['Roll']),
                                                   radians(o['Pitch']),
                                                   radians(o['Yaw']))
                self.euler_msg.yaw = o['Yaw']
                self.euler_msg.pitch = o['Pitch']
                self.euler_msg.roll = o['Roll']
            except KeyError:
                pass
            try:
        ...
        if self.pub_euler:
            self.euler_msg.header = self.h
            if self.euler_pub is None:
                self.euler_pub = rospy.Publisher('euler', mti, queue_size=10)
